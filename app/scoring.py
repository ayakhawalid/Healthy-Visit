...
def interpret_score(percentage: float) -> str:
    """
    Map a 0–100 percentage score to a grade label.
    """
    if percentage >= 85:
        return "Excellent"
    if percentage >= 70:
...
def score_answers(
    ...
    """
    Compute total and per-domain scores.

    Returns:
        total_score: sum of all selected option scores.
        max_score: theoretical maximum score.
        percentage: total_score / max_score * 100.
        domain_scores: mapping domain -> {raw_score, max_score}.
    """
    total_score = 0
    max_score = 0
    ...
    for q in QUESTIONNAIRE.questions:
        ...
        if selected_index is None:
            # Unanswered: treat as worst option (index 0)
            selected_index = 0
