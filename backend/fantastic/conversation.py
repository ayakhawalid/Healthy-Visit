...
class SessionState:
    ...
    language: str = "en"
    mode: Literal["collecting", "post_results_chat"] = "collecting"
...
def handle_chat_message(
    ...
    if state.mode == "post_results_chat" and state.last_score_summary:
        ...
        try:
            ...
            messages = [{"role": "system", "content": ADVICE_SYSTEM_PROMPT + "\n\n" + score_context}]
            for msg in history:
                if hasattr(msg, 'type'):
                    if msg.type == "human":
    ...
    if state.conversation_memory:
        for msg in state.conversation_memory.chat_memory.messages[-6:]:
    ...
    try:
        messages = [{"role": "system", "content": system_message.format(agent_prompt=next_agent_prompt)}]
        if state.conversation_memory:
